...
unsafe extern "C" fn game_specialairn4(fighter: &mut L2CAgentBase) {
let entry_id = WorkModule::get_int(fighter.module_accessor, *FIGHTER_INSTANCE_WORK_ID_INT_ENTRY_ID) as usize;

    if macros::is_excute(fighter) {
        WorkModule::set_int64(fighter.module_accessor, hash40("special_air_n4") as i64, *FIGHTER_ELIGHT_INSTANCE_WORK_ID_INT_ESWORD_INHERIT_OPEN_MOTION_KIND);
    }
    if macros::is_excute(fighter) {
...
pub fn install() {
    Agent::new("eflame")
        .game_acmd("game_specialairn4", game_specialairn4, Priority::Low)
        .install();
}
