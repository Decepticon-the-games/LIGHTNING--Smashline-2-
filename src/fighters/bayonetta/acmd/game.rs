use super::*;


 mod game_Attack11;
 mod game_Attack12;
 mod game_Attack13;
 mod game_Attack100End; 
 mod game_AttackAirN;
 mod game_AttackAirF;
 mod game_AttackAirF2;
 mod game_AttackAirF3;
 mod game_AttackAirB;
 mod game_AttackAirHi;
 mod game_AttackAirLw;
 mod game_LandingAirLw;
 mod game_AttackDash;
 mod game_AttackS3;
 mod game_AttackS32; 
 mod game_AttackS33;  
 mod game_AttackHi3;
 mod game_AttackLw3;
 mod game_AttackS4;
 mod game_AttackS4Hi;
 mod game_AttackS4Lw; 
 mod game_AttackHi4;
 mod game_AttackLw4;

 mod game_SpecialNEndF;
 mod game_SpecialNEndH;
 mod game_SpecialAirNEndF;
 mod game_SpecialAirNEndH;

 mod game_SpecialS;
 mod game_SpecialSHoldEnd;
 mod game_SpecialAirSD;
 mod game_SpecialAirSDLanding;
 mod game_SpecialAirSU;

 mod game_SpecialHi;
 mod game_SpecialAirHi;

 mod game_SpecialLw;
 mod game_SpecialAirLw;
 
 mod game_ThrowF;
 mod game_ThrowB;
 mod game_ThrowHi;
 mod game_ThrowLw;
 
 
  pub fn install() {
     
     game_Attack11::install();
     game_Attack12::install();
     game_Attack13::install();
     game_Attack100End::install(); 
     game_AttackAirN::install();
     game_AttackAirF::install();
     game_AttackAirF2::install();
     game_AttackAirF3::install();
     game_AttackAirB::install();
     game_AttackAirHi::install();
     game_AttackAirLw::install();
     game_LandingAirLw::install();
     game_AttackDash::install();
     game_AttackS3::install();
     game_AttackS32::install(); 
     game_AttackS33::install();  
     game_AttackHi3::install();
     game_AttackLw3::install();
     game_AttackS4::install();
     game_AttackS4Hi::install();
     game_AttackS4Lw::install(); 
     game_AttackHi4::install();
     game_AttackLw4::install();
    
     game_SpecialNEndF::install();
     game_SpecialNEndH::install();
     game_SpecialAirNEndF::install();
     game_SpecialAirNEndH::install();
    
     game_SpecialS::install();
     game_SpecialSHoldEnd::install();
     game_SpecialAirSD::install();
     game_SpecialAirSDLanding::install();
     game_SpecialAirSU::install();
    
     game_SpecialHi::install();
     game_SpecialAirHi::install();
    
     game_SpecialLw::install();
     game_SpecialAirLw::install();
     
     game_ThrowF::install();
     game_ThrowB::install();
     game_ThrowHi::install();
     game_ThrowLw::install();
 
 }



