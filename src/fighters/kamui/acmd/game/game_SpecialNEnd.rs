...
unsafe extern "C" fn game_dhspecialnend1(fighter: &mut L2CAgentBase) {
...
unsafe extern "C" fn game_dhspecialnend2(fighter: &mut L2CAgentBase) {
    let entry_id = WorkModule::get_int(fighter.module_accessor, *FIGHTER_INSTANCE_WORK_ID_INT_ENTRY_ID) as usize;
...
unsafe extern "C" fn game_dhspecialnendmax(fighter: &mut L2CAgentBase) {
    let entry_id = WorkModule::get_int(fighter.module_accessor, *FIGHTER_INSTANCE_WORK_ID_INT_ENTRY_ID) as usize;
...
pub fn install() {
    Agent::new("kamui_dragonhand")
        .game_acmd("game_dhspecialnendmax", game_dhspecialnendmax, Priority::Low)
        .install();
}
