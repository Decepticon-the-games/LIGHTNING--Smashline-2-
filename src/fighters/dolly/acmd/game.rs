use super::*;

 mod game_Attack11;
 mod game_Attack12;
 mod game_Attack13;
 mod game_AttackAirN;
 mod game_AttackAirF;
 mod game_AttackAirB;
 mod game_AttackAirHi;
 mod game_AttackAirLw;
 mod game_AttackDash;
 mod game_AttackS3;
 mod game_AttackHi3;
 mod game_AttackLw3;
 mod game_AttackS4;
 mod game_AttackHi4;
 mod game_AttackLw4;

 mod game_SpecialN;
 mod game_SpecialAirN;

 mod game_SpecialSFEnd;
 mod game_SpecialAirSFEnd;

 mod game_SpecialHiFall;
 mod game_SpecialAirHiEnd;

 mod game_SpecialLwEnd;
 //mod game_SpecialAirLw;

 mod game_SuperSpecial;
 mod game_SuperSpecial2;

 mod game_ThrowF;
 mod game_ThrowB;
 mod game_ThrowHi;
 mod game_ThrowLw;
 
 
  pub fn install() {

     game_Attack11::install();
     game_Attack12::install();
     game_Attack13::install();
     game_AttackAirN::install();
     game_AttackAirF::install();
     game_AttackAirB::install();
     game_AttackAirHi::install();
     game_AttackAirLw::install();
     game_AttackDash::install();
     game_AttackS3::install();
     game_AttackHi3::install();
     game_AttackLw3::install();
     game_AttackS4::install();
     game_AttackHi4::install();
     game_AttackLw4::install();
    
     game_SpecialN::install();
     game_SpecialAirN::install();
    
     game_SpecialSFEnd::install();
     game_SpecialAirSFEnd::install();
    
     game_SpecialHiFall::install();
     game_SpecialAirHiEnd::install();
    
     game_SpecialLwEnd::install();
     //game_SpecialAirLw::install();
    
     game_SuperSpecial::install();
     game_SuperSpecial2::install();
    
     game_ThrowF::install();
     game_ThrowB::install();
     game_ThrowHi::install();
     game_ThrowLw::install();
 
 }



